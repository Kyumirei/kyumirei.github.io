...
describe("GalleryService — getCategories", () => {
    ...
    it("groupe les images par sous-dossier", () => {
        ...
        assert.equal(result.find((c) => c.slug === "traditionelle")!.images.length, 2);
        assert.equal(result.find((c) => c.slug === "digitale")!.images.length, 1);
    });
    ...
    it("trie les catégories par préfixe numérique (ordre naturel)", () => {
        ...
        assert.deepEqual(
            svc.getCategories().map((c) => c.slug),
            ["01_a", "2_b", "10_z"]
        );
    });
